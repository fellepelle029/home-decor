    ...
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (this.type) {
        if (this.type === 'height') {
          ...
          this.from = +this.activeParams.heightFrom! || null;
          this.to = +this.activeParams.heightTo! || null;
        } else if (this.type === 'diameter') {
          this.open = !!(this.activeParams.diameterFrom || this.activeParams.diameterTo);
          this.from = +this.activeParams.diameterFrom !|| null;
          this.to = +this.activeParams.diameterTo! || null;
        }
      } else {
        if (this.categoryWithTypes && this.categoryWithTypes.types && this.categoryWithTypes.types.length > 0) {
          this.open = this.categoryWithTypes.types.some(type =>
            this.activeParams.types.includes(type.url)
          );
        }
      }
    });
